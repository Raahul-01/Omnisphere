...
def analyze_topic(topic):
    """Use Gemini API to analyze the topic and suggest a category and content type"""
    headers = {"Content-Type": "application/json"}
    payload = {
        "contents": [{
            "parts": [{
                "text": (
                    f"Analyze the following trending topic and suggest a category and content type:\n"
                    f"Topic: {topic}\n\n"
                    "Output format:\n"
                    "Category: <category>\n"
...
def generate_with_gemini(topic, news_articles, category, content_type):
    ...
    payload = {
        "contents": [{
            "parts": [{
                "text": (
                    f"Write a detailed 1000-word article about {topic}.\n"
                    f"Category: {category}\n"
    ...
    try:
        ...
        logging.error(f"Gemini content generation failed: {str(e)}")
        return None
...
async def main():
    """Main execution flow"""
    global skipped_count  # Add this line to use the global variable
    try:
        validate_environment()
        trends = await fetch_trends()  # Add await here
        if not trends:
        ...
        for idx, topic in enumerate(trends, 1):
            try:
                ...
                # Force process if too many topics have been skipped
                ...
                category, content_type = analyze_topic(topic)
                if not category or not content_type:
                    logging.warning(f"Failed to analyze topic: {topic}")
                    continue
                
                # Generate more compelling headline
                engaging_title = generate_viral_headline(topic, news_articles)
                
                # Generate content with enhanced context
                content = generate_with_gemini(topic, news_articles, category, content_type)
                if not content:
                    logging.warning("Gemini failed. Trying DeepSeek...")
                    content = generate_with_deepseek(topic, news_articles, category, content_type)
                
                if content:
                    image_url = fetch_google_image(topic)
                    store_content_in_firebase(topic, content, category, content_type, image_url)
                    logging.info(f"Successfully generated content with headline: {engaging_title}")
                else:
